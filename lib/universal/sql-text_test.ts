...
/**
 * `markdownLink()` tests and usage examples to demonstrate how to compose complex SQL.
 *
 * Purpose:
 *   `markdownLink(textExpr, urlExpr)` shows how to wrap small SQL text-generation
 *   conveniences on top of your lower-level primitives (`sqlCat`, `sqlRaw`, `SQL`).
 *   It builds a Markdown-style `[text](url)` expression while:
 *     - accepting algebraic fragments (string | SQL | sqlRaw | arrays of those)
 *     - URL-encoding the URL exactly once via `sqlpage.url_encode(...)`
 *     - delegating string assembly to `sqlCat` so concatenation is clean and readable
 *
 * Notes:
 *   - This test suite doubles as documentation. See each subtest for examples.
 *   - `markdownLink()` returns a *plain SQL string expression* (not a parameterized `SQL`).
 */
Deno.test("markdownLink() — complex examples & usage", async (t) => {
  await t.step("basic: sqlCat label + sqlCat url (auto-encodes once)", () => {
  ...
  await t.step(
    "idempotent encoding: url already encoded stays single-encoded",
    () => {
      // Caller used sqlRaw to build an already-encoded expression.
  ...
  await t.step("identifiers via sqlRaw (Option A style)", () => {
    // Treat identifiers as raw SQL tokens:
    const text = sqlRaw`${"regime_label"}`; // → regime_label
    const url = sqlRaw`'/r?x=' || ${sqlRaw`${"id"}`}`; // → '/r?x=' || id
  ...
  await t.step(
    "with SQL fragments: .text() is spliced (values become literals)",
    () => {
      // Build a function call with a parameter that becomes a literal in .text():
      const textFrag = SQL`coalesce(${sqlRaw`${"name"}`}, ${"N/A"})`;
  ...
  await t.step("arrays: mixed fragments are flattened with ' || '", () => {
    // Reminder: strings are treated as raw SQL tokens. If you need literals, quote them.
    const textParts = [
      sqlRaw`${"first_name"}`,
      sqlRaw`' '`,
      sqlRaw`${"last_name"}`,
  ...
  await t.step("nested arrays & nesting with sqlCat", () => {
    const text = [
      sqlCat`${"org_name"}`, // → org_name
      sqlRaw`' — '`,
      [sqlRaw`${"dept"}`, sqlRaw`'/'`, sqlRaw`${"team"}`], // → dept || '/' || team
    ];
    const url = [
      sqlRaw`'/org?o='`,
      sqlRaw`${"org_id"}`,
  ...
  await t.step("documentation: minimal wrapper pattern", () => {
    // This subtest is illustrative: it shows how you can wrap common textual
    // patterns into small helpers on top of `sqlCat`, `sqlRaw`, and `SQL`.
    //
    // The point of `anchor()` is *not* power, but *clarity*: hide small bits
    // of ceremony (e.g., `[`/`](` /`)` pieces and URL-encoding) while still
    // accepting algebraic fragments (strings, sqlRaw, SQL, arrays).
    //
    // If you later need, you can build more wrappers (e.g., image tags, HTML spans)
    // using the same approach—compose primitives, keep them small, test them well.
